...
def test_model(model,test_data,optimizer,criterion):
    ...
    for f_name,y in tqdm(zip(name_list,correctness_list)):
        ...
        feats_l = combined_feats[:,0,:,:].unsqueeze(1)
        feats_r = combined_feats[:,1,:,:].unsqueeze(1)
       
        #print(feats_l.shape,feats_r.shape)
        ...
        # forward + backward + optimize
        # left channel
        output_l = model(feats_l.float())
        # right channel
        output_r = model(feats_r.float())
        
        # convert correctness value to tensor and normalize range 0 - 1
        y = format_correctness(y)
        
        outputs = (output_l + output_r)/2
        print(outputs,y)
...
def train_model(model,train_data,optimizer,criterion):
    ...
    for batch in tqdm(my_dataloader):
        ...
        # split into channels (could do this before but want features consistent)
        feats_l = feats[:,0,:,:].unsqueeze(1)
        feats_r = feats[:,1,:,:].unsqueeze(1)
        ...
        for x1,y1,x2,y2 in zip(output_l.detach().numpy(),scores_l.detach().numpy(),output_r.detach().numpy(),scores_r.detach().numpy()):
            print("L - P: %s | T: %s"%(x1,y1))
            print("R - P: %s | T: %s"%(x2,y2))
        
        loss_l = criterion(output_l, scores_l)
        loss_r = criterion(output_r, scores_r)
        loss = loss_l + loss_r
        loss.backward()
        optimizer.step()
