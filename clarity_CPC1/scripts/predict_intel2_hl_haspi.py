...
def audio_pipeline(path,fs=32000):
    ...
    resampler = Resample(orig_freq=fs, new_freq=44100)
...
def train_model(model,train_data,optimizer,criterion):
    ...
    for name,corr,scene,lis in  zip(name_list,correctness_list,scene_list,listener_list):
        train_dict[name] = {"signal": name,"correctness":corr,"scene": scene,"listener":lis}
    ...
    train_set.set_output_keys(["wav","clean_wav", "formatted_correctness","audiogram_np"])
    my_dataloader = DataLoader(train_set,1,collate_fn=sb.dataio.batch.PaddedBatch)
   
    for batch in tqdm(my_dataloader):
        wavs,wavs_clean, correctness,audiogram = batch
        ...
        scores = batch_HASPI(wavs_clean.data,wavs.data,audiogram)

        feats = compute_feats(wavs.data,44100) #B *  C * T
        ...
        """
        for x,y in zip(outputs.detach().numpy(),scores):
            print("P: %s | T: %s"%(x,y))
        loss = criterion(outputs, torch.Tensor(scores).unsqueeze(0))
        loss.backward()
        optimizer.step()
